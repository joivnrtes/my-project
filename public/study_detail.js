let postId = null;
    ...
    function fetchPostDetail(){
      fetchWithAuth(`http://localhost:3000/api/study/post/${postId}`)
        .then(data => {
          if(!data.success){
            alert("帖子不存在或已被删除");
            goBack();
    ...
    function likePost(pid){
      fetchWithAuth(`http://localhost:3000/api/study/post/${pid}/like`, {
        method: 'POST'
      })
      .then(data => {
        if(!data.success){
    ...
    async function submitComment(){
      ...
      // 再发评论请求
      fetchWithAuth(`http://localhost:3000/api/study/post/${postId}/comment`, {
        method: 'POST',
    ...
    function deleteComment(commentId){
      ...
      fetchWithAuth(`http://localhost:3000/api/study/post/${postId}/comment/${commentId}`, {
        method: 'DELETE'
      })
      .then(data => {
        if(!data.success){
