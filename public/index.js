...
function initProvinceCitySelects() {
  ...
  fetch('https://websocket-server-o0o0.onrender.com/api/auth/all')
  ...
  if (selectedProvince && allProvinceCityData[selectedProvince]) {
    ...
    function fetchAllGyms() {
      fetch('https://websocket-server-o0o0.onrender.com/api/gym/all')
        .then(response => response.json())
        .then(data => {
  ...
  fetchWithAuth('https://websocket-server-o0o0.onrender.com/api/community/posts')
    .then(data => {
      if (!data.success) {
...
function likeCommunityPost(postId, button) {
  fetchWithAuth(`https://websocket-server-o0o0.onrender.com/api/community/post/${postId}/like`, {
    method: "POST",
...
function renderStudyPosts() {
  // 从后端获取所有学习帖
  fetchWithAuth('https://websocket-server-o0o0.onrender.com/api/study/posts')
    .then(data => {
      if (!data.success) {
...
function likeStudyPost(postId, btn) {
  fetchWithAuth(`https://websocket-server-o0o0.onrender.com/api/study/post/${postId}/like`, {
    method: 'POST',
    headers: { "Content-Type": "application/json" }
  })
    .then(data => {
...
// ========== 好友逻辑 ==========
  ...
  fetch(`https://websocket-server-o0o0.onrender.com/api/user/search?keyword=${encodeURIComponent(username)}`)
...
function loadFriendRequests() {
  fetchWithAuth('https://websocket-server-o0o0.onrender.com/api/friend-request/received')
    .then(result => {
      if (!result.success) {
      ...
      requests.forEach(req => {
        ...
        img.src = req.from.avatarUrl || 'https://websocket-server-o0o0.onrender.com/default-avatar.png';
        img.alt = '头像';
...
if (!userId) {
  ...
  fetchWithAuth(`https://websocket-server-o0o0.onrender.com/api/user/${userId}/friends`)
    .then(result => {
      if (!result.success) {
      ...
      friends.forEach(friend => {
        ...
        // 创建第一个 div，用于显示头像和用户名
          ...
          // 创建头像 img
          const img = document.createElement('img');
          img.src = friend.avatarUrl || 'https://websocket-server-o0o0.onrender.com/default-avatar.png';
          img.alt = '头像';
          img.style.width = '40px';
          img.style.height = '40px';
          img.style.borderRadius = '50%';
...
chatBtn.classList.add("chat-btn"); // ✅ 添加类名，方便查找
chatBtn.dataset.friendId = friend._id; // ✅ 绑定好友 ID
chatBtn.style.position = 'relative'; 

// ✅ 创建小红点
const unreadBadge = document.createElement('span');
unreadBadge.classList.add("unread-badge"); // ✅ 确保类名正确
...
function openFriendInfo(event, targetUserId) {
  ...
  fetchWithAuth(`https://websocket-server-o0o0.onrender.com/api/user/${targetUserId}`)
    .then(result => {
      ...
      document.getElementById('friend-info-avatar').src = friendData.avatarUrl || 'https://websocket-server-o0o0.onrender.com/default-avatar.png';
  ...
  fetchWithAuth(`https://websocket-server-o0o0.onrender.com/api/friend-request/${friendId}`, {
    method: 'DELETE',
    headers: {
...
fetchWithAuth(`https://websocket-server-o0o0.onrender.com/api/chat/history?friendId=${friendId}`)
  .then(data => {
    ...
    console.log("📂 加载聊天记录, 删除的消息 ID:", deletedMessages);
  ...
  .catch(err => console.error('获取聊天记录失败:', err));
    ...
    function createBubble(text, type, timestamp, msgId) {
      const bubble = document.createElement('div');
      bubble.classList.add('chat-bubble', type);
    
      if (msgId) {
        bubble.dataset.id = msgId; // ✅ 绑定消息 ID
      } else {
        console.warn("⚠️ 缺少消息 ID，删除功能可能无法正常工作:", text);
      }
      ...
      timeEl.textContent = isNaN(dateObj.getTime()) ? "" : dateObj.toLocaleTimeString();
...
function deleteChatHistory() {
  ...
  // ✅ 1. 获取所有消息 ID（修正选择器 `.chat-bubble`）
  let deletedChats = JSON.parse(localStorage.getItem("deletedChats")) || {};
  let deletedMessages = Array.from(document.querySelectorAll("#chat-messages .chat-bubble"))
    .map(msg => msg.dataset.id)
    .filter(id => id); // ✅ 过滤掉 `undefined` 值

  console.log("🗑 删除的消息 ID:", deletedMessages);

  // ✅ 2. 存入 `localStorage`
  deletedChats[currentChatUser] = deletedMessages;
  localStorage.setItem("deletedChats", JSON.stringify(deletedChats));

  console.log("✅ 存入 localStorage: ", JSON.stringify(deletedChats));

  // ✅ 3. 仅从前端 UI 移除消息
  document.getElementById('chat-messages').innerHTML = '';

  // ✅ 4. 发送 WebSocket 消息（可选）
...
function renderChatHistory(messages) {
  ...
  let deletedChats = JSON.parse(localStorage.getItem("deletedChats")) || {};
  let deletedMessages = deletedChats[currentChatUser] || [];

  console.log("🚀 该好友的聊天记录已删除，仅前端隐藏:", deletedMessages);

  messages.forEach(message => {
    if (!message._id) {
      console.warn("⚠️ 服务器返回的消息没有 ID:", message);
      return;
    }

    // ✅ 过滤掉已删除的消息
    if (deletedMessages.includes(message._id)) {
      console.log("🚫 跳过已删除的消息:", message._id);
      return;
    }

    const bubbleType = message.from._id.toString() === getCurrentUserId() ? 'me' : 'friend';
    const bubble = createBubble(message.message, bubbleType, message.timestamp, message._id);
